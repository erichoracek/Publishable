...
extension AnyPropertyPublisherTests {
    ...
    fileprivate final class MainActorObject {
        @ObservationPublished @ObservationIgnored
        var storedProperty = 0
...
extension AnyPropertyPublisherTests {
    ...
    @Publishable @Observable
    fileprivate final class PlatformConditionalObject {
        #if os(macOS)
            @ObservationPublished @ObservationIgnored
            var storedProperty = 0
        #endif
    ...
    @Test
    func testPlatformConditionalPublisher() {
        var object: PlatformConditionalObject? = .init()
        var publishableQueue = [Int]()
        nonisolated(unsafe) var observationsQueue: [Void] = []
...
extension AnyPropertyPublisherTests {
    ...
    @Publishable @Observable
    fileprivate final class PrivatePublishedObject {
        @ObservationPublished @ObservationIgnored
        private var storedProperty = 0

        @ObservationPublished @ObservationIgnored
        var unrelatedProperty = 0

        var privateStoredPropertyPublisher: some Publisher<Int, Never> {
            publisher.storedProperty
        }

        var privateStoredPropertyPassthrough: Int {
            get { storedProperty }
    ...
    @Test
    func testPrivatePublisher() {
        var object: PrivatePublishedObject? = .init()
        var publishableQueue = [Int]()
        nonisolated(unsafe) var observationsQueue: [Void] = []
        ...
        func observe() {
            withObservationTracking {
                _ = object?.privateStoredPropertyPublisher
            } onChange: {
                observationsQueue.append(())
            }
        }
        ...
        object?.privateStoredPropertyPassthrough = 0
        #expect(publishableQueue.popFirst() == nil)
        #expect(observationsQueue.popFirst() == nil)

        object?.privateStoredPropertyPassthrough += 1
        #expect(publishableQueue.popFirst() == 1)
        #expect(observationsQueue.popFirst() != nil)
